...
async def get_all_company_and_employer(
    user: user_dependency, company_id: int, employers_id: int
):
    companies_query = (
...
async def get_talonario(
    user: user_dependency, company_id: int, employers_id: int, period_id: int
):
    companies_query = (
        session.query(Employers, Companies)
        .join(Companies, onclause=Companies.id == company_id)
