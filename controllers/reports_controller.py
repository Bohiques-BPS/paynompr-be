...
def all_counterfoil_controller(company_id, period_id ):
    ...
    for all_time_query in all_time_query_sums:
        ...
        all_times_query = session.query(Time).select_from(Period).join(Time, Period.id == Time.period_id and Time.employer_id == employer_id).filter(Period.year == 2024,Time.employer_id == employer_id,Period.period_start <= period.period_start).all()
